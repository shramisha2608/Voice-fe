...
const MIN_DATE = parseISO("2000-01-01");
...
const commonDatePickerProps = {
  ...
  slots: { openPickerButton: CalendarPickerButton, openPickerIcon: null },
  slotProps: {
    textField: {
      size: "small",
      ...
      inputProps: {
        ...
        inputMode: "none",      // hides mobile keyboard
        onKeyDown: (e) => e.preventDefault(), // block keystrokes
        onPaste: (e) => e.preventDefault(),   // block paste
      },
    },
    // (optional, v6) also tell the field to be readOnly
    field: { readOnly: true },
    popper: {
...
const DateRangeFilter = ({
  ...
  const handleStartChange = (newStart) => {
    if (!newStart || (endDate && isAfter(newStart, endDate))) return;
    onStartDateChange(newStart);
  };

  const handleEndChange = (newEnd) => {
    if (!newEnd || (startDate && isBefore(newEnd, startDate))) return;
    onEndDateChange(newEnd);
  };

  return (
    <Box className="date-range-filter" gap={2} sx={{ mb: 2, ...sx }}>
      {[
        {
          label: "From",
          value: startDate,
      ...
      ].map(({ label, ...pickerProps }) => (
        <Box key={label}>
          <Typography variant="body2" sx={{ color: "var(--text-color)", mb: 0.5 }}>
            {label}
          </Typography>
          <DatePicker {...commonDatePickerProps} {...pickerProps} views={['year', 'month', 'day']} />
        </Box>
      ))}
    </Box>
  );
